...
Sessions also support `using` declarations: disposal calls `end()` when the scope exits, so a session can be bound to a block instead of a manual call. The declaration needs the engine to provide `Symbol.dispose`, which Hermes and other React Native engines may not; there, `end()` does the same work.
