...
Construction copies the key into a session-owned snapshot; the caller's buffer is not modified. Locking [zeroes](/concepts/security-model/#what-the-library-handles) the session's copy and is permanent: a locked session throws on signing, there is no unlock, and new signatures require a new session. An unlock path would mean the key still existed somewhere after `lock()`.
